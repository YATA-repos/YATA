...
class OrderService {
  ...
  Future<(Order?, bool)> checkoutCart(
    ...
    final Order? updatedOrder = await _orderRepository
        .updateById(cartId, <String, dynamic>{
  ...
  Future<(Order?, bool)> cancelOrder(
    ...
    final Order? canceledOrder = await _orderRepository.updateById(
      orderId,
      <String, dynamic>{
  ...
  Future<Map<String, dynamic>> getOrderHistory(
    ...
    if (request.dateFrom != null || request.dateTo != null) {
      ...
      for (final Order order in orders) {
        if (request.dateFrom != null &&
            order.orderedAt.isBefore(request.dateFrom!)) {
          continue;
        }
        if (request.dateTo != null &&
            order.orderedAt.isAfter(request.dateTo!)) {
          continue;
        }
    ...
    if (request.menuItemName != null) {
      ...
      for (final Order order in orders) {
        final List<OrderItem> items = await _orderItemRepository.findByOrderId(
          order.id!,
        );
  ...
  Future<Map<String, dynamic>?> getOrderWithItems(
    ...
    for (final OrderItem item in orderItems) {
      ...
      itemsWithMenu.add(<String, dynamic>{
        "order_item": item,
        "menu_item": menuItem,
      });
    }
  ...
  Future<OrderCalculationResult> _calculateOrderTotal(
    ...
    final int subtotal = orderItems.fold(
      0,
  ...
  /// 注文に対する材料消費を実行
  Future<void> _consumeMaterialsForOrder(
    ...
    for (final OrderItem item in orderItems) {
      ...
      for (final dynamic recipe in recipes) {
        if (!(recipe.isOptional as bool)) {
          final double requiredAmount =
              (recipe.requiredAmount as double) * item.quantity;
          materialConsumption[recipe.materialId as String] =
              (materialConsumption[recipe.materialId as String] ?? 0.0) +
              requiredAmount;
        }
      }
    }
    ...
    for (final MapEntry<String, double> entry in materialConsumption.entries) {
      ...
      if (material != null) {
        final double newStock =
            (material.currentStock as double) - consumedAmount;
        await _materialRepository.updateStockAmount(
          materialId,
          newStock,
          userId,
        );
  ...
  /// 注文キャンセル時の材料在庫復元
  Future<void> _restoreMaterialsFromOrder(
    List<OrderItem> orderItems,
    String userId,
  ) async {
      ...
      for (final dynamic recipe in recipes) {
        if (!(recipe.isOptional as bool)) {
          ...
          materialRestoration[recipe.materialId as String] =
              (materialRestoration[recipe.materialId as String] ?? 0.0) +
              restoredAmount;
        }
      }
    }
    ...
    for (final MapEntry<String, double> entry in materialRestoration.entries) {
      ...
      if (material != null) {
        final double newStock =
            (material.currentStock as double) + restoredAmount;
        await _materialRepository.updateStockAmount(
          materialId,
          newStock,
          userId,
