...
class Logger {
  ...
  void setGlobalLevel(LogLevel level) => _core.updateConfig((LogConfig c) => c.copyWith(globalLevel: level));
  void setTagLevel(String tag, LogLevel level) =>
      _core.updateConfig((LogConfig c) => c.copyWith(tagLevels: <String, LogLevel>{...c.tagLevels, tag: level}));
...
class _LoggerCore {
  _LoggerCore._() {
    ...
    _configHub = LogConfigHub(
      LogConfig.defaults(fileDirPath: ""),
    );
  ...
  Future<void> _process(_Pending p) async {
    ...
    final LogEvent ev = LogEvent(
      ...
      err: p.error == null
          ? null
          : <String, String>{
              "type": p.error.runtimeType.toString(),
              "message": p.error.toString(),
            },
  ...
  void _maybeEmitRateSummary(LogEvent e) {
    ...
    final _RateSummary s = _rateSummaries.putIfAbsent(key, () => _RateSummary());
    s.suppressed++;
    if (s.lastSummaryAt == null || now.difference(s.lastSummaryAt!) >= _rateConfig.summaryInterval) {
      s.lastSummaryAt = now;
  ...
  void _handleUnhandled(Object error, StackTrace? st, {required bool handledZone}) {
    ...
    final _CrashBucket bucket = _crashDedup.putIfAbsent(h, () => _CrashBucket());
    if (bucket.first == null || now.difference(bucket.first!) > window) {
      ...
      log(
        ...
        error: error,
        st: st,
        fields: () => <String, dynamic>{'crash': <String, dynamic>{'handled': 'unhandled', 'zone': handledZone}},
      );
    } else {
      bucket.suppressed++;
