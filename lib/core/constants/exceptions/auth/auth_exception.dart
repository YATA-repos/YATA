...
class AuthException extends BaseContextException<AuthError> {
  ...
  /// 認証初期化失敗例外の作成
  factory AuthException.initializationFailed(String error) => AuthException(
  ...
  /// ユーザー情報取得失敗例外の作成
  factory AuthException.userInfoFetchFailed(String error) => AuthException(
    AuthError.userInfoFetchFailed,
    params: <String, String>{"error": error},
  );

  /// ログアウト失敗例外の作成
  factory AuthException.logoutFailed(String error) => AuthException(
    AuthError.logoutFailed,
    params: <String, String>{"error": error},
  );

  /// 無効プロバイダー例外の作成
  factory AuthException.invalidProvider(String provider) => AuthException(
    AuthError.invalidProvider,
    params: <String, String>{"provider": provider},
  );
  ...
  /// 認証キャンセル例外の作成
  factory AuthException.authenticationCancelled() => AuthException(
    AuthError.authenticationCancelled,
  );

  /// ネットワークエラー例外の作成
  factory AuthException.networkError(String error) => AuthException(
