...
class LogService {
  ...
  /// 情報レベルログ（事前定義メッセージ使用）
  static void infoWithMessage(String component, LogMessage logMessage, [Map<String, String>? params]) {
    final message = params != null ? logMessage.withParams(params) : logMessage.combinedMessage;
    _log(LogLevel.info, component, message, null);
  }

  /// 警告レベルログ（事前定義メッセージ使用）
  static void warningWithMessage(String component, LogMessage logMessage, [Map<String, String>? params]) {
    final message = params != null ? logMessage.withParams(params) : logMessage.combinedMessage;
    _log(LogLevel.warning, component, message, null);
  }

  /// エラーレベルログ（事前定義メッセージ使用）
  static void errorWithMessage(String component, LogMessage logMessage, [Map<String, String>? params, Object? error, StackTrace? stackTrace]) {
    final message = params != null ? logMessage.withParams(params) : logMessage.combinedMessage;
  ...
  static void _log(
    ...
    String component,
    String message,
    [String? messageJa, Object? error, StackTrace? stackTrace]
  ) {
    // 最小ログレベルチェック
    ...
    // メッセージフォーマット
    final formattedMessage = _formatMessage(message, messageJa);
  ...
  static void _addToBuffer(
    ...
    final logEntry = _formatLogEntry(level, component, message, timestamp, error, stackTrace);
  ...
  static String _formatLogEntry(
    ...
    entry.writeln('[$timestamp] [$component] [${level.value.toUpperCase()}] $message');
  ...
  static Future<void> _writeBufferedEntry(String entry) async {
    try {
      ...
      final filename = '$dateStr-mixed.log';
  ...
  static Future<Map<String, dynamic>> getLogStats() async {
    if (_logDirectory == null) {
      return {'error': 'Log directory not initialized'};
    }
