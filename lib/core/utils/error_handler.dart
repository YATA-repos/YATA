...
class ErrorHandler with LoggerMixin {
  ...
  @override
  String get loggerComponent => "ErrorHandler";
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        ...
        action: SnackBarAction(
          ...
          textColor: Theme.of(context).colorScheme.onError,
          onPressed: isRetryable ? onRetry : () => ScaffoldMessenger.of(context).hideCurrentSnackBar(),
        ),
      ),
    );
  }
  ...
  Never handleServiceError(String operation, dynamic error, [StackTrace? stackTrace]) {
    logError("Service error in $operation", error, stackTrace);
