...
void main() {
  ...
  loggerProvider.overrideWith((Ref ref) {
    ...
    final CapturedLog summaryLog = logger.entries.firstWhere(
      (CapturedLog entry) => entry.message.contains("Order total calculated"),
    );

    expect(summaryLog.level, Level.debug);
    expect(summaryLog.tag, equals("OrderCalculationService"));
