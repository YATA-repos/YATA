...
### [Feature] 商品のサイズバリエーション機能を実装する
- **ID**: Menu-Feature-38
- **Priority**: P1
- **Size**: L
- **Area**: Menu
- **Dependencies**: None
- **Goal**: メニュー管理画面で新たに商品の"サイズ"バリエーションを追加・編集できるようになり、注文時にサイズ選択が可能になる。
- **Steps**:
  1. `./docs/draft/add_size_to_menu.md` の要件定義を確認し、実装仕様を確定する。
  2. メニュー・サイズバリエーションのデータモデルと API契約を設計する。
  3. バックエンド側でサイズバリエーション関連のDBスキーマとAPIを実装する。
  4. メニュー管理UI でサイズバリエーション追加・編集フォームを実装する。
  5. 注文画面でサイズ選択オプションの表示と処理を実装する。
  6. 統合テストと動作確認を実施する。
- **Description**: 商品のサイズバリエーション（例: 小・中・大など）を追加できるようにし、注文時にユーザーがサイズを選択できるようにする機能。`./docs/draft/add_size_to_menu.md`にて要件定義・調査が進行中。

### [Enhancement] 注文状況画面と在庫管理画面にリアルタイム機能を導入する
- **ID**: Infra-Enhancement-39
- **Priority**: P2
- **Size**: L
- **Area**: Core
- **Dependencies**: None
- **Goal**: 注文状況画面と在庫管理画面で、他のユーザーやデバイスからの変更がリアルタイムで反映されるようになる。
- **Steps**:
  1. 既存の他の画面で実装されているリアルタイム機能の実装パターンを調査し、整理する。
  2. SupabaseのリアルタイムSubscription設定と監視対象テーブルを確認・設計する。
  3. 注文状況画面にリアルタイムリスナーを追加し、注文データ変更を自動反映する。
  4. 在庫管理画面にリアルタイムリスナーを追加し、在庫データ変更を自動反映する。
  5. UIの更新フロー（キャッシュ無効化など）を実装する。
  6. 動作確認とストレステストを実施する。
- **Description**: 複数ユーザーや複数デバイスからの同時アクセスに対応するため、注文状況画面と在庫管理画面にもリアルタイム機能を導入し、データ変更が即座に画面に反映されるようにする。
...
### [Enhancement] カテゴリの重複名エラーを適切に処理・表示する
- **ID**: Inventory-Enhancement-40
- **Priority**: P1
- **Size**: M
- **Area**: Inventory
- **Dependencies**: None
- **Goal**: メニュー・在庫カテゴリの名前に UNIQUE制約が設定されており、重複するカテゴリ名でのエラーが返ってきた際に、ユーザーに対して適切にエラーを表示し、再命名を促す UI が実装される。
- **Steps**:
  1. バックエンド API からの UNIQUE制約違反エラーレスポンスを確認し、エラーコード・メッセージフォーマットを整理する。
  2. `CategoriesManagementController` のエラーハンドリングロジックを実装し、UNIQUE違反を検出する。
  3. UI側で分かりやすいエラーメッセージダイアログを表示し、ユーザーに再命名を促すようにする。
  4. テストケースを追加して、エラー表示と再命名フロー全体を検証する。
- **Description**: メニュー・在庫カテゴリのテーブル設計において名前に UNIQUE制約を追加した。重複する名前でのカテゴリ作成・更新時に、バックエンドから UNIQUE制約違反エラーが返ってくるため、これをクライアント側で捕捉し、ユーザーに対して分かりやすいエラーメッセージを表示し、再命名を促す処理を実装する必要がある。
